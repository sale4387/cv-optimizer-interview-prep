...
def _render_legacy_prep(
    ...
    st.caption(
        "Legacy interview preparation. "
        "Create a new application to use "
...
def _generation_button(
    *,
    ...
    label = (
        ...
        if retry
        else "Generate interview prep"
    )

    if st.button(
        label,
        key=(
            "generate_interview_prep_"
            f"{application_id}"
        ),
        type="primary",
        width="stretch",
...
def render_application_interview_prep(
    ...
    prep = get_existing_interview_prep(
        app
    )
    ...
    st.caption(
        "Generated on demand from the job ad, "
        "optimized CV, fit/gap analysis and "
        "stored Company & Market Intelligence."
    )

    if prep is None:
        st.info(
            "Interview preparation has not "
            "been generated yet."
        )
